...
% Calculate transfer function Hs(s) = (zs-zu)/żr
for i = 1:length(w)
    s = 1i*w(i);
...
% Configure plot
xlabel('Frequency (Hz)')
ylabel('Suspension Deflection (dB)')
title('Frequency Response of Suspension Deflection Transfer Function Hs(s)')
ylim([-160 0])  % Adjusted based on your image
xlim([1e-3 1e3])
...
% High frequency asymptote (-60 dB/decade)
f_high = logspace(1, 3, 100);
mag_high = -60*log10(f_high/f_high(1)) + mag_db(find(f >= f_high(1), 1));
...
text(1e-2, -40, '+20 dB/decade', 'Color', 'r', 'FontSize', 10)
text(1e2, -100, '-60 dB/decade', 'Color', 'r', 'FontSize', 10)
text(f1, -20, 'Primary Ride: 1.25 Hz', 'FontSize', 10)
text(f2, -40, 'Secondary Ride: 10.9 Hz', 'FontSize', 10)
